...
class LSHAttention(nn.Module):
    ...
    def forward(self,
        ...
        buckets = self.hash_vectors(n_buckets,
                                    qk,
                                    key_namespace=depth,
                                    fetch=is_reverse,
        ...
        ticker = torch.arange(total_hashes * seqlen,
        ...
        if input_attn_mask is not None:
            input_attn_mask = F.pad(input_attn_mask,
                                    (0, seqlen - input_attn_mask.shape[-1], 0,
                                     seqlen - input_attn_mask.shape[-2]),
                                    value=True)
            dot_attn_indices = ((bq_t * seqlen)[:, :, :, None] +
                                bkv_t[:, :, None, :])
        ...
        if not self._allow_duplicate_attention:
            ...
            dup_counts = chunked_sum(dup_counts,
                                     chunks=(total_hashes * batch_size))
        ...
        if self._return_attn:
            ...
            attn_unsort = attn_unsort.view(batch_size * total_hashes,
                                           -1).long()
            unsorted_dots = torch.zeros(batch_size * total_hashes,
                                        seqlen * seqlen,
                                        device=device)
...
class LSHSelfAttention(nn.Module):
    def __init__(self,
        ...
        self.local_attn = LocalAttention(window_size=bucket_size * 2,
                                         causal=causal,
                                         dropout=dropout,
                                         shared_qk=True,
    ...
    def forward(self,
        ...
        attn_fn_in_chunks = process_inputs_chunk(partial_attn_fn,
                                                 chunks=self.attn_chunks)

        out, attn, buckets = attn_fn_in_chunks(qk, v, **masks)

        if self.callback is not None:
            self.callback(attn.reshape(b, lsh_h, t, -1),
                          buckets.reshape(b, lsh_h, -1))
...
class LinearPositionalEmbedding(nn.Module):
    ...
    def forward(self, x):
        t = torch.arange(x.shape[1], device=x.device) / self.max_seq_len
        #t = t.repeat(x.shape[0], x.shape[2], 1).permute(0, 2, 1)
        t = t.repeat(x.shape[0], 1, 1).permute(0, 2, 1)
        # print(t)
        return t
...
class ReformerLM(nn.Module):
    def __init__(self,
        ...
        elif linear_position_emb:
            self.pos_emb = LinearPositionalEmbedding(max_seq_len)
